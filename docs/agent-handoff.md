...
- `npx electron out/main/index.js --task=import-platform --platformCode=baemin`
  - 통과
  - `숨김피자 검증A / 1,100원` 반영 상태와 원복 후 `숨김피자 / 1,000원` 상태를 각각 재수집으로 확인
- 이번 정리에서 수정한 핵심
  ...
  - 배민 숨김/품절 메뉴 검색 보강
    - 메뉴 상태가 `숨김`이면 검색 전 `숨김` 필터, `품절`이면 `오늘만 품절` 필터를 먼저 맞추도록 보강
...
- 현재 배민 반복 검증용 안전 대상은 `숨김피자`다. 삭제 테스트를 걸기 전에는 사용 의도를 다시 확인하고, 원복 가능성을 먼저 계산한다.
