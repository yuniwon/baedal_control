...
- 렌더러 UX 1차 단순화가 들어갔다.
  ...
  - `연결 / 기록`은 `고급 기능 보기`를 눌렀을 때만 보인다.
  - `가져오기` 화면의 브라우저 검사는 기본 숨김이며 `브라우저 진단 보기`로 펼친다.
- 렌더러 UX 2차 단순화도 반영됐다.
  - `홈`은 플랫폼 상태와 지금 할 일만 먼저 보이고, 최근 변화는 `최근 변화 보기`로 펼친다.
  - `메뉴`는 기준 메뉴명/가격과 핵심 상태만 먼저 보이고, 원본 상세는 `원본 상세 보기`로 펼친다.
- 렌더러 UX 3차 단순화도 반영됐다.
  - `옵션`은 항목 구성과 상태만 먼저 보이고, 원본 그룹별 연결 메뉴/마지막 확인은 `원본 연결 보기`로 펼친다.
  - `가져오기`는 최근 가져오기 제목과 상태만 먼저 보이고, 긴 설명은 `최근 결과 보기`로 펼친다.
  - 읽은 화면 패널은 같은 카드 아래로 펼쳐지는 구조라 화면이 좌우로 갈라지지 않는다.
- 렌더러 UX 4차 단순화도 반영됐다.
  - `매핑`은 기준 메뉴와 현재 연결 상태를 먼저 보이고, 후보의 긴 원본 바인딩/가격 variant 상세는 상단 `상세 보기`에서만 펼친다.
  - `실행 기록`은 가져오기 기록과 반영 기록 모두 상태 요약만 먼저 보이고, 긴 실패 문맥과 항목별 상세는 각 행의 `상세 보기`에서만 펼친다.
- 렌더러 UX 5차 정리도 반영됐다.
  - 고급 화면 이름을 실제 작업 언어 기준으로 `연결 / 기록`으로 줄였다.
  - `HistoryPage`는 초기 import 기록도 주입할 수 있어, 비동기 마운트 경고 없이 테스트할 수 있다.
- 렌더러 UX 6차 정리도 반영 중이다.
  - 가져오기 버튼 문구를 `저장하고 읽기 / 다시 읽기 / 읽는 중`으로 정리했다.
  - 최근 가져오기 기본 요약은 짧은 한 줄만 먼저 보이고, 전체 설명은 `최근 결과 보기`에서 펼친다.
  - `수집 검사` 명칭을 `읽은 화면`으로 바꿔 실제 읽은 페이지와 필드 확인에 집중시켰다.
  - 미리보기 대화상자는 `반영 확인`으로 바꾸고, 한 줄 변경 요약과 `반영값`만 먼저 보여 주도록 정리했다.
...
- 이번 정리에서 수정한 핵심
  - 매핑/실행 기록 화면 정보 밀도 4차 축소
    - 매핑: 기준 메뉴와 현재 연결을 먼저 보고, 후보/현재 연결의 긴 원본 요약과 가격 variant 줄 목록은 `상세 보기`로만 펼침
    - 실행 기록: 가져오기 설명과 항목별 실패 문맥을 각 행의 `상세 보기`로만 펼침
    - 관련 렌더러 테스트 추가/보정 후 통과 확인
  - 옵션/가져오기 화면 정보 밀도 3차 축소
    - 옵션: 원본 그룹 카드와 연결 메뉴 상세를 기본 접힘으로 전환
    - 가져오기: 최근 가져오기 설명을 기본 접힘으로 전환하고, 읽은 화면 패널을 카드 하단 확장형으로 정리
    - 관련 렌더러 테스트 추가/보정 후 통과 확인
  - 홈/메뉴 화면 정보 밀도 2차 축소
    - 홈: `플랫폼 상태`와 `최근 변화`를 분리하고, 최근 변화는 기본 접힘으로 전환
    - 메뉴: 원본 카드의 긴 바인딩/가격 variant/옵션그룹/마지막 확인 시각은 기본 접힘으로 전환
    - 관련 렌더러 테스트 추가/보정 후 통과 확인
  ...
  - 읽기 전용 실행 제안 레이어 추가
    ...
    - `반영 확인` 미리보기 대화상자도 같은 공용 요약 helper를 사용해 이름/가격/가격 구조 변경을 같은 방식으로 보여 줌
    - 긴 다중 가격 요약은 기본 상태에서 접고, `상세 보기/접기`로만 전체 전후 비교를 펼치게 해 미리보기 스캔성을 높임
    - 실제 운영 DB 기준 스모크 검증 완료
