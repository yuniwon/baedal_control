...
- 렌더러 UX 3차 단순화도 반영됐다.
  ...
  - `가져오기`는 최근 가져오기 제목과 상태만 먼저 보이고, 긴 설명은 `최근 가져오기 보기`로 펼친다.
  - 수집 검사는 같은 카드 아래로 펼쳐지는 구조라 화면이 좌우로 갈라지지 않는다.
- 렌더러 UX 4차 단순화도 반영됐다.
...
- 이번 정리에서 수정한 핵심
  ...
  - 옵션/가져오기 화면 정보 밀도 3차 축소
    ...
    - 가져오기: 최근 가져오기 설명을 기본 접힘으로 전환하고, 수집 검사 패널을 카드 하단 확장형으로 정리
    - 관련 렌더러 테스트 추가/보정 후 통과 확인
  - 홈/메뉴 화면 정보 밀도 2차 축소
  ...
  - 읽기 전용 실행 제안 레이어 추가
    ...
    - `변경 예정` 미리보기 대화상자도 같은 공용 요약 helper를 사용해 이름/가격/가격 구조 변경을 같은 방식으로 보여 줌
